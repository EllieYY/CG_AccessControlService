...
public class PortInfo extends OperationResult {
    ...
    private void setReaderAlarm(int alram, CGPortInfo port) {
        port.setInvalidCardAlarm((alram & 0x01) > 0 ? true : false);
        port.setStrikeAlarm((alram & 0x02) > 0 ? true : false);
        port.setForcedOpenAlarm((alram & 0x03) > 0 ? true : false);
        port.setHeldOpenAlarm((alram & 0x08) > 0 ? true : false);
        port.setBlacklistAlarm((alram & 0x10) > 0 ? true : false);
        port.setTamperReaderAlarm((alram & 0x20) > 0 ? true : false);
    }
}
