...
public class CardInfo extends Operation {
     ...
     *  ic\id     3~4byte
     */
    @CmdProp(index = 0, len = 9)
    private String cardNo;

    // BCD码
    @CmdProp(index = 1, len = 4)
    private String pwd;

    // BCD码，2099-12-31 23:59:59 ==> 9912312359
    @CmdProp(index = 2, len = 5)
    private Date validDate;

    // 从左到右，对应1~4号门
    @CmdProp(index = 3, len = 4)
    private List<Integer> timeTz;

    // 0 - 无权限 最大值-不受限制
    @CmdProp(index = 4, len = 2)
     ...
     *  3    节假日受限制
     */
    @CmdProp(index = 5, len = 1)
    private int expAlvl;

    // 0 - 正常  1 - 挂失  2 - 黑名单
    @CmdProp(index = 6, len = 1)
    private int status;

    // 从左到右对应门，节假日编号，1~32
    @CmdProp(index = 7, len = 4)
